...
class RobotsTxtChecker {
    cache = new Map();
    async checkAllowed(url, userAgent = '*') {
        try {
            const urlObj = new URL(url);
    ...
    isPathAllowed(rules, path, userAgent) {
        const relevantRules = rules.filter(rule => rule.userAgent === '*' || rule.userAgent.toLowerCase().includes(userAgent.toLowerCase()));
        let allowed = true;
...
class SessionManager {
    ...
    static defaultSessionId = 'default';
    ...
    static async closeAllSessions() {
        const closePromises = Array.from(this.sessions.keys()).map(id => this.closeSession(id).catch(console.warn));
    ...
    static async cleanupOldSessions(maxAge = 60 * 60 * 1000) {
        const now = Date.now();
        const oldSessions = [];
        this.sessions.forEach((session, id) => {
...
(function (CrawlStrategy) {
    ...
    CrawlStrategy["BEST_FIRST"] = "bestfirst";
...
async function mockCrawl(url, config) {
...
async function mockDeepCrawl(startUrl, config) {
...
export async function crawlUrl(config) {
    // Validate input
...
export async function deepCrawl(config) {
    // Validate input
    if (!config.url || !isValidUrl(config.url)) {
        throw new Error(`Invalid URL: ${config.url}`);
    }
    ...
    try {
        ...
        while (urlsToCrawl.length > 0 && crawledUrls.size < maxPages && currentDepth < depth) {
                ...
                case 'bestfirst':
                    ...
                    if (keywords.length > 0) {
                        urlsToCrawl.sort((a, b) => {
                            const scoreA = keywords.reduce((score, keyword) => score + (a.toLowerCase().includes(keyword.toLowerCase()) ? 1 : 0), 0);
                            const scoreB = keywords.reduce((score, keyword) => score + (b.toLowerCase().includes(keyword.toLowerCase()) ? 1 : 0), 0);
            ...
            for (const url of urlsToProcessThisDepth) {
                if (crawledUrls.has(url) || crawledUrls.size >= maxPages)
                    break;
                try {
...
export async function analyzeContent(config) {
    ...
    try {
        ...
        const analysis = {
            ...
            links: extractLinks ? (result.links?.map((link) => ({
                url: link.url,
                text: link.text || link.url,
                type: link.url.startsWith(config.url) ? 'internal' : 'external'
            })) || []) : null,
            images: extractImages ? (result.images?.map((img) => ({
                src: img.src,
                alt: img.alt || '',
