...
async function testCrawl4AI() {
  try {
    ...
    console.log("Title:", response[0].metadata?.title);
    
    // Extract links
    let allLinks: string[] = [];
    if (response[0].links) {
      const internalLinks = response[0].links.internal?.map((link: any) => link.href) || [];
      const externalLinks = response[0].links.external?.map((link: any) => link.href) || [];
